...
`python` falso no lugar da consulta ao banco: exit 0 = a lista ja traz o historico (o CSV
nao e lido); exit 3 = historico ainda nao carregado no banco (a lista + o CSV); exit 1 =
sem banco (volta ao CSV e diz). O md5 e o de modo texto (\\r removido), igual ao que o job
grava no banco.
"""
...
def test_banco_responde_e_a_lista_dele_mais_o_csv_e_a_memoria(tmp_path):
    ...
    falso = f"python() {{ echo {md5_velho}; echo {md5_novo}; return 3; }}"
...
def test_banco_sem_historico_ainda_respeita_o_csv(tmp_path):
    ...
    arvore, controle, md5_velho, md5_novo = _preparar(tmp_path)
    saida = _rodar(tmp_path, "banco", "python() { return 3; }", arvore, controle)   # banco vazio
...
def test_com_o_historico_no_banco_o_csv_nao_entra(tmp_path):
    """Exit 0 do listar_md5 --com-historico: a lista do banco ja traz o historico e o CSV nao
    e consultado. Prova: o CSV diz que o NOVO ja foi tratado, e mesmo assim ele aparece."""
    arvore, controle, md5_velho, md5_novo = _preparar(tmp_path)
    controle.write_text(controle.read_text() + "NOVO.RET," + md5_novo + "\n", encoding="utf-8")
    falso = (f'python() {{ [ "$2" = "--com-historico" ] || return 1; echo {md5_velho}; return 0; }}')
    saida = _rodar(tmp_path, "banco", falso, arvore, controle)
    assert "controle: fonte BANCO (1 md5 registrados, com o historico no banco)" in saida
    assert saida.strip().splitlines()[-1] == str(arvore), "o NOVO.RET e novidade: o CSV ficou de fora"
