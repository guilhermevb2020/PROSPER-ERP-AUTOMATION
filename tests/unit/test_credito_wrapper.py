"""O Hub deve receber o codigo do robo, mesmo com stdout passando por tee."""
import os
...
def test_tee_nao_esconde_falha_do_robo(tmp_path, codigo):
    ...
    env={**os.environ,'TEST_EXIT_CODE':str(codigo),'LOGROBO':str(tmp_path/'credito.log'),'TMPDIR':str(tmp_path)}
