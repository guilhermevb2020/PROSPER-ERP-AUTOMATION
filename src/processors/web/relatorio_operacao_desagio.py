...
class ProcessadorRelatorioDesagio:
    ...
    async def verificar_e_resolver_recaptcha(self):
        ...
        try:
            ...
            if captcha_em_nova_aba:
                ...
                # Aguardar MAIS TEMPO para o site processar o CAPTCHA e preparar o download
                print(f"[CAPTCHA] Aguardando 5s após clicar Confirmar (site processando)...")
                await asyncio.sleep(5)

                # Voltar para tab principal ANTES de fechar a aba
                self.tab = tab_principal
                ...
                try:
                    # Tentar fechar a aba do CAPTCHA de forma mais robusta
                    if tab_captcha and tab_captcha in self.browser.tabs:
                        await tab_captcha.close()
                        print(f"[CAPTCHA] ✅ Aba do CAPTCHA fechada")
                    else:
                        print(f"[CAPTCHA] ⚠️ Aba do CAPTCHA já foi fechada")
                except Exception as e:
                    print(f"[CAPTCHA] ⚠️ Erro ao fechar aba: {e}")
                    # Ignorar erro - já estamos na tab principal
    ...
    async def aguardar_download(self):
        ...
        # Capturar timestamp de modificação dos arquivos existentes
        timestamp_inicio = time.time()

        # Aguardar novo arquivo aparecer OU arquivo existente ser modificado
        for i in range(TIMEOUT_DOWNLOAD):
            await asyncio.sleep(1)

            # Procurar arquivos CSV modificados APÓS o início da espera
            for arquivo in DOWNLOAD_DIR.glob("*.csv"):
                # Checar se arquivo foi modificado DEPOIS do timestamp_inicio
                if arquivo.stat().st_mtime > timestamp_inicio:
