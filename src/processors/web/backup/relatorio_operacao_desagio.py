...
class ProcessadorRelatorioDesagio:
    ...
    async def verificar_e_resolver_recaptcha(self):
        ...
        try:
            ...
            if captcha_em_nova_aba:
                ...
                # Aguardar download iniciar ANTES de fechar aba
                print(f"[CAPTCHA] Aguardando 5s para download iniciar...")
                await asyncio.sleep(5)

                # Voltar para tab principal MAS NÃO FECHAR aba do CAPTCHA ainda
                self.tab = tab_principal
                print(f"[CAPTCHA] ✅ Voltou para tab principal (aba CAPTCHA ainda aberta)")
                await asyncio.sleep(2)
    ...
    async def executar_ciclo_extracao(self):
        ...
        try:
            ...
            # ETAPA 4: Preencher formulário (JÁ CLICA NO BOTÃO IMPRIMIR)
            await self.preencher_formulario()

            # ETAPA 4.5: Verificar e resolver CAPTCHA (se aparecer após clicar)
            await self.verificar_e_resolver_recaptcha()

            # ETAPA 5: Gerar relatório - PULADO (botão já clicado na etapa 4)
