...
# VERSÃO: v2.0 - PROCESSADOR DE RELATÓRIO DE OPERAÇÃO DESÁGIO
# DESCRIÇÃO: Extrai relatório de operações de deságio do SmartSecurities usando Nodriver
...
class ProcessadorRelatorioDesagio:
    """Processador de Relatório de Operação Deságio - 100% Nodriver Async"""
    ...
    def __init__(self):
        ...
        self.display = os.environ.get('DISPLAY', ':1')  # Display :1 (padrão)
        ...
        # CapSolver API para resolução de CAPTCHA
...
"""
    ...
    async def navegar_relatorio_desagio(self):
        """Navega até o relatório de Detalhamento do Deságio e preenche os filtros"""
        from datetime import datetime, timedelta

        print(f"\n{'='*80}")
        print("📊 NAVEGANDO PARA RELATÓRIO DE DESÁGIO")
        print(f"{'='*80}\n")
        ...
        try:
            ...
            # 1. CLICAR NO LINK "Detalhamento do Deságio"
            print(f"[DESÁGIO] Procurando link 'Detalhamento do Deságio'...")

            click_result_raw = await self.tab.evaluate("""
                (() => {
                    // Função para procurar link em um documento
                    function procurarLink(doc) {
                        // Procurar link com name="sub27"
                        const link = doc.querySelector('a[name="sub27"]');
                        if (link) {
                            link.click();
                            return { success: true, found: 'link com name=sub27', location: 'document' };
                        }

                        // Alternativa: procurar por texto
                        const links = Array.from(doc.querySelectorAll('a'));
                        const linkDesagio = links.find(a => a.textContent.includes('Detalhamento do Deságio') || a.textContent.includes('Detalhamento do Desagio'));
                        if (linkDesagio) {
                            linkDesagio.click();
                            return { success: true, found: 'link por texto', location: 'document' };
                        }
                    ...
                    const resultado = procurarLink(document);
                    ...
                    for (let i = 0; i < iframes.length; i++) {
                        try {
                            ...
                            const resultado = procurarLink(iframeDoc);
                            if (resultado) {
                                resultado.location = `iframe[${i}] (${iframes[i].src || iframes[i].name || 'sem-nome'})`;
                                return resultado;
                            }
                        } catch (e) {
                    ...
                    return { success: false, error: 'Link não encontrado em nenhum frame', totalFrames: iframes.length };
                })()
            """)

            # Converter para dict (nodriver pode retornar lista)
            click_result = self.converter_cdp_para_dict(click_result_raw)

            print(f"[DESÁGIO] Resultado do clique: {click_result}")

            if not click_result.get('success'):
                # Se não encontrou o link, tentar navegar direto para a URL
                print(f"[DESÁGIO] Link não encontrado, navegando diretamente para URL...")
                url_desagio = "https://www.smartsecurities.com.br/smart/relatorios/frmrelatorio.php?page=relatorios/detalhamentodesagio.php"
                await self.tab.get(url_desagio)
                print(f"[DESÁGIO] Navegou para: {url_desagio}")

            # Aguardar página do relatório carregar (tempo maior)
            await asyncio.sleep(10)

            # 2. PREENCHER DATAS
            print(f"[DESÁGIO] Preenchendo campos de data...")

            # Calcular datas - data inicial = hoje, data final = hoje
            data_final = datetime.now()
            data_inicial = datetime.now()  # Data de hoje conforme solicitado

            data_inicial_str = data_inicial.strftime("%d/%m/%Y")
            data_final_str = data_final.strftime("%d/%m/%Y")

            print(f"[DESÁGIO]   Data Inicial: {data_inicial_str}")
            print(f"[DESÁGIO]   Data Final: {data_final_str}")

            # Preencher campos via JavaScript (procurando em todos os frames)
            preencher_result_raw = await self.tab.evaluate(f"""
                (() => {{
                    // Função para procurar e preencher campos em um documento
                    function preencherCampos(doc) {{
                    ...
                    // 1. Procurar no documento principal
                    let resultado = preencherCampos(document);
                    if (resultado) return resultado;
                    ...
                    for (let i = 0; i < iframes.length; i++) {{
                        try {{
                            const iframeDoc = iframes[i].contentDocument || iframes[i].contentWindow.document;
                            resultado = preencherCampos(iframeDoc);
                            if (resultado) {{
                                resultado.location = `iframe[${{i}}] (${{iframes[i].src || iframes[i].name || 'sem-nome'}})`;
                                return resultado;
                            }}
                    ...
                    return {{ success: false, error: 'Campos de data não encontrados em nenhum frame', totalFrames: iframes.length }};
                }})()
            """)

            # Converter para dict
            preencher_result = self.converter_cdp_para_dict(preencher_result_raw)

            print(f"[DESÁGIO] Resultado preenchimento datas: {preencher_result}")

            if not preencher_result.get('success'):
                raise Exception("Não foi possível preencher os campos de data")

            # 3. SELECIONAR RADIO BUTTON CSV
            print(f"[DESÁGIO] Selecionando formato CSV...")

            csv_result_raw = await self.tab.evaluate("""
                (() => {
                    // Função para procurar e selecionar radio button em um documento
                    function selecionarCSV(doc) {
                        const radioCSV = doc.querySelector('input[name="tipoImp"][value="csvType"]');
                        if (radioCSV) {
                            radioCSV.checked = true;
                            return { success: true, checked: radioCSV.checked, location: 'document' };
                        }
                        return null;
                    }

                    // 1. Procurar no documento principal
                    let resultado = selecionarCSV(document);
                    if (resultado) return resultado;
                    ...
                    for (let i = 0; i < iframes.length; i++) {
                        try {
                            const iframeDoc = iframes[i].contentDocument || iframes[i].contentWindow.document;
                            resultado = selecionarCSV(iframeDoc);
                            if (resultado) {
                                resultado.location = `iframe[${i}] (${iframes[i].src || iframes[i].name || 'sem-nome'})`;
                                return resultado;
                            }
                    ...
                    return { success: false, error: 'Radio button CSV não encontrado em nenhum frame', totalFrames: iframes.length };
                })()
            """)

            # Converter para dict
            csv_result = self.converter_cdp_para_dict(csv_result_raw)

            print(f"[DESÁGIO] Resultado seleção CSV: {csv_result}")

            if not csv_result.get('success'):
                raise Exception("Não foi possível selecionar formato CSV")

            # 4. CLICAR EM "GERAR RELATÓRIO"
            print(f"[DESÁGIO] Clicando em 'Gerar relatório'...")

            gerar_result_raw = await self.tab.evaluate("""
                (() => {
                    // Função para procurar e clicar no botão em um documento
                    function clicarBotao(doc) {
                        const botaoGerar = doc.querySelector('input[name="Imprimir"]');
                        if (botaoGerar) {
                            botaoGerar.click();
                            return { success: true, location: 'document' };
                        }
                        return null;
                    }

                    // 1. Procurar no documento principal
                    let resultado = clicarBotao(document);
                    if (resultado) return resultado;
                    ...
                    for (let i = 0; i < iframes.length; i++) {
                        try {
                            const iframeDoc = iframes[i].contentDocument || iframes[i].contentWindow.document;
                            resultado = clicarBotao(iframeDoc);
                            if (resultado) {
                                resultado.location = `iframe[${i}] (${iframes[i].src || iframes[i].name || 'sem-nome'})`;
                                return resultado;
                            }
                    ...
                    return { success: false, error: 'Botão Gerar não encontrado em nenhum frame', totalFrames: iframes.length };
                })()
            """)

            # Converter para dict
            gerar_result = self.converter_cdp_para_dict(gerar_result_raw)

            print(f"[DESÁGIO] Resultado clique gerar: {gerar_result}")

            if not gerar_result.get('success'):
                raise Exception("Não foi possível clicar no botão Gerar")

            # Aguardar processamento
            await asyncio.sleep(10)

            print(f"\n{'='*80}")
            print("✅ RELATÓRIO DE DESÁGIO GERADO COM SUCESSO!")
            print(f"{'='*80}\n")

        except Exception as e:
            print(f"\n{'='*80}")
            print(f"❌ ERRO AO GERAR RELATÓRIO DE DESÁGIO: {e}")
            print(f"{'='*80}\n")
            raise
    ...
    async def fazer_login_automatico(self):
        ...
        try:
            ...
            # 🔄 NAVEGAR PARA RELATÓRIO DE DESÁGIO
            await self.navegar_relatorio_desagio()
...
"""
    ...
    async def processar(self, modo_simulacao=False):
        ...
        try:
            ...
            print(f"[API] ✅ API iniciada na porta {os.getenv('SERVER_PORT', '6090')}")
